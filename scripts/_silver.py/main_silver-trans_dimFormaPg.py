...
HEADER_PREFIX = "forma_pagamento;tipo_pagamento"
...
def list_bronze_tables_formapgto(engine: Engine) -> list[str]:
    sql = text(
        """
        SELECT tablename
        FROM pg_tables
        WHERE schemaname = '_bronze'
          AND tablename LIKE '%\\_formapgto' ESCAPE '\\'
        ORDER BY tablename;
        """
    )
...
def derive_ano_mes_from_table(table_name: str) -> str:
    m = re.search(r"t_(\d{6})_formapgto$", table_name)
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Normaliza nomes de colunas vindos do CSV:
    - remove espaços
    - minúsculas
    - troca espaços por underscore
    - remove BOM se vier na primeira coluna
    """
    df = df.copy()
    df.columns = [
...
def cast_types(df: pd.DataFrame) -> pd.DataFrame:
    """
    Converte tipos para o padrão Silver.
    - pax -> inteiro (nullable Int64)
    """
    df = df.copy()

    if "pax" in df.columns:
        s = df["pax"].astype("string").str.strip()

        # se o arquivo vier com separador BR, aproveita sua função:
        # s = to_decimal_br(s)  # <- se pax puder vir com "1.234" ou "1,234"
        # Mas como pax é quantidade, normalmente basta isso:
        df["pax"] = pd.to_numeric(s, errors="coerce").astype("Int64")
...
def main() -> None:
    ...
    silver_table = os.getenv("SILVER_TABLE_DIMPF", "s_dim_formapgto").strip()
    write_mode = os.getenv("WRITE_MODE", "append").strip().lower()

    ensure_silver_table(engine, silver_schema, silver_table)

    bronze_tables = list_bronze_tables_formapgto(engine)
    if not bronze_tables:
        logging.warning("Nenhuma tabela _formapgto encontrada no schema _bronze.")
        return
    ...
    logging.info("Encontradas %d tabelas _formapgto para consolidar.", len(bronze_tables))
