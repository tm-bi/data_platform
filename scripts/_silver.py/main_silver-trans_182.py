...
HEADER_PREFIX = "Conta;Loc VHF;Autorização;Cliente;"

STOP_MARKERS = (
    '"";"";',
    '"Total Geral"',
    '"Total";',
    "Produtos/Combos vendidos por período",
    "Produto;Qtd;Total",
)
...
def list_bronze_tables_182(engine: Engine) -> list[str]:
    sql = text(
        """
        SELECT tablename
        FROM pg_tables
        WHERE schemaname = '_bronze'
          AND tablename LIKE '%\\_182' ESCAPE '\\'
        ORDER BY tablename;
        """
    )
...
def derive_ano_mes_from_table(table_name: str) -> str:
    m = re.search(r"t_(\d{6})_182$", table_name)
...
def cast_types(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "dt_baixa" in df.columns:
        df["dt_baixa"] = pd.to_datetime(df["dt_baixa"], dayfirst=True, errors="coerce").dt.date
...
def ensure_silver_table(engine: Engine, schema: str, table: str) -> None:
    with engine.begin() as conn:
        ...
        conn.execute(text(f'ALTER TABLE "{schema}"."{table}" ADD COLUMN IF NOT EXISTS teste TEXT;'))
...
def main() -> None:
    ...
    silver_table = os.getenv("SILVER_TABLE_182", "novaxs_182").strip()
    write_mode = os.getenv("WRITE_MODE", "append").strip().lower()

    ensure_silver_table(engine, silver_schema, silver_table)

    bronze_tables = list_bronze_tables_182(engine)
    if not bronze_tables:
        logging.warning("Nenhuma tabela _182 encontrada no schema _bronze.")
        return
    ...
    logging.info("Encontradas %d tabelas _182 para consolidar.", len(bronze_tables))
