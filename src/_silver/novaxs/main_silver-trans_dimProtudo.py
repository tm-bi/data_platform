...
HEADER_PREFIX = "produto;tipo_produto;categoria;infantil;"
...
def list_bronze_tables_produto(engine: Engine) -> list[str]:
    sql = text(
        """
        SELECT tablename
        FROM pg_tables
        WHERE schemaname = '_bronze'
          AND tablename LIKE '%\\_produto' ESCAPE '\\'
        ORDER BY tablename;
        """
    )
...
def derive_ano_mes_from_table(table_name: str) -> str:
    m = re.search(r"t_(\d{6})_produto$", table_name)
...
def main() -> None:
    ...
    silver_table = os.getenv("SILVER_TABLE_DIMP", "s_dim_produto").strip()
    write_mode = os.getenv("WRITE_MODE", "append").strip().lower()

    ensure_silver_table(engine, silver_schema, silver_table)

    bronze_tables = list_bronze_tables_produto(engine)
    if not bronze_tables:
        logging.warning("Nenhuma tabela _produto encontrada no schema _bronze.")
        return
    ...
    logging.info("Encontradas %d tabelas _produto para consolidar.", len(bronze_tables))
