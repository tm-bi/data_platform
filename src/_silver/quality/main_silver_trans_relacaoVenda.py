...
HEADER_PREFIX = "Sequência;Período;Cupom;"
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    rename_map = {
        ...
        "Venda Pré Pago": "venda_pre_pago",
        "Total da Venda": "ttl_venda"
    }
...
def cast_types(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "periodo" in df.columns:
        ...
        # fallback para variações (caso venha sem segundos, etc.)
        m = dt.isna() & s.notna()
        if m.any():
            dt.loc[m] = pd.to_datetime(s[m], dayfirst=True, errors="coerce")

        df["dt_venda"] = dt.dt.strftime("%Y-%m-%d").astype("string")
        df["hr_venda"] = dt.dt.strftime("%H:%M:%S").astype("string")

        df.loc[dt.isna(), "dt_venda"] = pd.NA
        df.loc[dt.isna(), "hr_venda"] = pd.NA

        df = df.drop(columns=["periodo"])

    if "ttl_venda" in df.columns:
        s = (
            df["ttl_venda"]
            .replace({"": pd.NA, "NULL": pd.NA, "null": pd.NA})
            .astype("string")
        ...
        df["ttl_venda"] = pd.to_numeric(s, errors="coerce").round(2)
...
def main() -> None:
    ...
    bronze_table = os.getenv("SRC_TABLE_VENDAS")
    silver_table = os.getenv("SILVER_TABLE_VENDAS")
